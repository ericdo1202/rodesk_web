...
if ( ! class_exists( 'Quiety_Theme_Helper' ) ) {
	class Quiety_Theme_Helper {
		...
		static function quiety_breadcrumb() {
			...
			if ( ! is_home() ) {
				...
				if ( is_category() || is_single() ) {
					...
					echo '</span>';
				} elseif ( QUIETY_WOOCOMMERCE_IS_ACTIVE && is_shop() ) {
					...
					<?php if ( $logo_contrast !== '' && $is_sticky_header == true ): ?>
						<img <?php if ( ! empty( $retina_logo_sticky ) ) {
							echo 'srcset="' . esc_attr( $retina_logo_sticky ) . ' 2x"';
								...
								alt="<?php bloginfo( 'name' ); ?>" class="logo-sticky">
					<?php endif; ?>
		...
		public static function get_footers_types() {
			$footer = ['' => esc_html__( 'Default', 'quiety' ) ];
			$footers = get_posts(
				[
					'posts_per_page' => - 1,
					'post_type'      => 'quiety_footer',
					'orderby'        => 'name',
					'order'          => 'ASC'
				]
			);
		...
		public static function quiety_render_footer($footer_style){
